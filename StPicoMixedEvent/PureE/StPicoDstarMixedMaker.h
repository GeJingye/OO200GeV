...
class StPicoDstarMixedMaker : public StMaker
{
  ...
  private: 
    TFile* mFile;
	// std::map<Key, T> 是 C++ 标准库提供的有序关联容器。建立“键→值”的一一映射，支持对数复杂度（O(log n)）的插入、查找、删除，且按键自动排序。
	std::map<Int_t, Int_t> mrunnum;
	string mRunNumList;
	// std::vector<T> 是 C++ 标准库提供的动态顺序容器。“可变长数组”，在尾部高效 O(1) 地插入 / 删除，支持随机访问 O(1)，内存连续。
    ...
    TH2F* h_dEdx_Pc;
	TH2F* h_dEdx_Pc__PIDcut;
	TH2F* h_dEdx_Pc__EIDcut;
	TH1F* h_m2;
    TH2F* h_m2_Pc;
    TH1F* h_pDca;
    TH1F* h_ppT;
	...
	// good track
	TH2F* h_nSigmaE_P__total;
	TH2F* h_nSigmaPi_P__total;
	TH2F* h_nSigmaK_P__total;
	TH2F* h_nSigmaP_P__total;

	TH3F* h_Pt_Cen_nSigmaE__PureE;
	TH3F* h_Eta_Cen_nSigmaE__PureE;
	TH3F* h_Phi_Cen_nSigmaE__PureE;
	TH3F* h_Pt_Eta_Phi__PureE;
	TH3F* h_Pt_Cen_nSigmaPi__PurePi;
	TH3F* h_Eta_Cen_nSigmaPi__PurePi;
	TH3F* h_Phi_Cen_nSigmaPi__PurePi;
	TH3F* h_Pt_Eta_Phi__PurePi;
	...
	TH2F* h_nSigmaEcorr_P__TOFMatch; // with 1/beta cut

	TH2F* h_nSigmaPion_P__2;
    TH2F* h_nSigmaKaon_P__2;
	TH2F* h_nSigmaPion_P__3;
    TH2F* h_nSigmaKaon_P__3;
    TH2F* h_nSigmaProton_P__2;
    
	TH2F* h_pT_Eta__EIDcut_3_lowP_e;
	TH2F* h_pT_Eta__EIDcut_3_lowP_p;
