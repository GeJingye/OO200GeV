...
void DrawNSigmaEPiKP(TString inFilename = "roots/27_20260304_TOF_corrEtaPhi_P24iy.root", Int_t number = 27) //
{
	...
	TH1F *h_cen__reWeight = (TH1F *)inFile->Get("h_cen__reWeight");
	...
	if (0) // h_passEvtcut,h_passTrkcut,h_nHitsFit_Pt_Eta,h_nHitsDEdx_Pt_Eta,h_pDca_Pt_Eta
	{
		...
		line_DCA1->Draw("same");
		TLine *line_DCA2 = new TLine(0.0, 3, 0.15, 3);
		line_DCA2->SetLineColor(kRed);
		line_DCA2->Draw("same");
		TLine *line_DCA3 = new TLine(0.15, 1, 0.15, 3);
	...
	if (0) // track QA and TOF track check
	{
		...
		h_pT__TOFMatch->Draw("HIST");

		c1->cd(5);
		gPad->SetLogy(1);
		// h_Eta__TOFMatch->GetYaxis()->SetRangeUser(1., 1e8);
		h_Eta__TOFMatch->Draw("HIST");
	...
	if (0) // check PhiV cut
	{
		...
		h_Mee__unlikeSame->DrawClone("HIST");
		h_Mee__unlikeSame__w_PhiV_Cut->DrawClone("same HIST");
